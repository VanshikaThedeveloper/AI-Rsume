...
//  const [isAiEdit, setAiEdit] = useState(false);
...
// const handleEnhanceSection = useCallback(
//   async (section) => {
//     setShowAIMenu(false);                       // menu hide immediately

//     try {

//        // ✅ Step 1: Auto-save latest state
//       await saveResumeToBackend();

//        // 1. pick raw content from current state
//     const rawContent = resumeData[section];

//     // 2. Always send string to backend
//     const content =
//       typeof rawContent === "string"
//         ? rawContent
//         : JSON.stringify(rawContent, null, 2);
...
//       // 3. Write improved content back into state
//       if (typeof rawContent === "string") {
//         // simple string field (summary / philosophy)
//         setResumeData((prev) => ({ ...prev, [section]: improved }));
//       } else {
//         // array sections ‑ replace first item’s accomplishment / description
//         setResumeData((prev) => {
//           const updated = [...prev[section]];
//           if (updated[0]) {
//             if (section === "experience") updated[0].accomplishment = improved;
...
// ----------------------------------sharebutton---------------------------------------
  ...
  try {
    ...
    // ✨ Map parsed data to existing resume format
    const mappedData = {
